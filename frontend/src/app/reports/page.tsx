	...
	return (
		<div className="container">
			<Stack direction="row" justify="space-between" align="center" style={{ marginBottom: 16 }}>
				<H1>Relatórios</H1>
				<Select value={selectedMonth} onChange={(e) => setSelectedMonth(e.target.value)} style={{ minWidth: 200 }}>
			...
			<Card style={{ marginBottom: 24 }}>
				...
				<table className="table">
					...
					<tbody>
						{monthlyTrends.map(
							(trend: {
								monthYear: React.Key | null | undefined;
								income: number;
								expense: number;
								balance: number;
							}) => (
			...
			<Card style={{ marginBottom: 24 }}>
				...
				<table className="table">
					...
					<tbody>
						{categoryBreakdown.slice(0, 10).map(
							(
								item: {
									category:
										| string
										...
										<td>
											<Text
												...
												style={{
													color: transaction.type === "income" ? "var(--success)" : "var(--danger)",
													background:
														transaction.type === "income" ? "rgba(61, 220, 151, 0.1)" : "rgba(255, 107, 107, 0.1)",
													padding: "2px 8px",
													borderRadius: 4,
													display: "inline-block",
												}}
