...
export const selectTopTransactions = createSelector(
  [selectReportData],
...
export const selectAverageDailySpending = createSelector(
  [selectReportData],
  (data) => data?.averageDailySpending || 0
);

export const selectIncomeCategories = createSelector(
  [selectCategoryBreakdown],
  (breakdown) => breakdown.filter(item => item.type === 'income')
);

export const selectExpenseCategories = createSelector(
  [selectCategoryBreakdown],
  (breakdown) => breakdown.filter(item => item.type === 'expense')
);
