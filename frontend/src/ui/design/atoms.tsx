...
export function Stack({ children, gap = 12, direction = 'column', align, justify, style }:{
  children: React.ReactNode; gap?: number; direction?: 'row'|'column';
  align?: React.CSSProperties['alignItems']; justify?: React.CSSProperties['justifyContent'];
  style?: React.CSSProperties;
}) {
  return (
    <div style={{ display: 'flex', gap, flexDirection: direction, alignItems: align, justifyContent: justify, ...style }}>
      {children}
    </div>
  );
}

export function Text({ children, size = 14, weight = 400, muted = false, style }:{
  children: React.ReactNode; size?: number; weight?: number; muted?: boolean; style?: React.CSSProperties;
}) {
  return <div style={{ fontSize: size, fontWeight: weight, color: muted ? 'var(--muted)' : 'inherit', ...style }}>{children}</div>;
}
...
export function Button({ children, variant = 'primary', onClick, type='button', disabled }:{
  children: React.ReactNode; variant?: 'primary'|'ghost'|'danger'; onClick?: () => void; type?: 'button'|'submit'; disabled?: boolean;
}) {
  ...
  return (
    <button type={type} disabled={disabled} onClick={onClick} style={{
      ...
      border, cursor: disabled ? 'not-allowed' : 'pointer', opacity: disabled ? 0.6 : 1
    }}>{children}</button>
  );
}
